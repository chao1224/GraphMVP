...
# for schnet_lr_scale in "${schnet_lr_scale_list[@]}"; do
# for dropout_ratio in "${dropout_ratio_list[@]}"; do
#      export folder="$mode"/"$dataset"/CL_"$alpha_1"_"$AE_model"_"$alpha_2"/"$num_interactions"_"$num_gaussians"_"$cutoff"_"$schnet_lr_scale"/"$CL_similarity_metric"_"$T"_"$normalize"_"$AE_loss"_"$detach"_"$beta"_"$epochs"_"$dropout_ratio"
...
#           echo "$output_file" undone
#      # sbatch --gres=gpu:v100l:1 -c 8 --mem=32G -t "$time":00:00  --account=rrg-bengioy-ad --qos=high --job-name=VAE \
#      # --output="$output_file" \
#      # ./run_pretrain_"$mode".sh \
#      # --epochs="$epochs" \
#      # --dataset="$dataset" \
