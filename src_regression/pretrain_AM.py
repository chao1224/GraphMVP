...
from util_complete_feature import MaskAtom
...
if __name__ == '__main__':
    ...
    if 'GEOM' in args.dataset:
        dataset = MoleculeDatasetComplete(
        ...
        dataset = MoleculeDatasetComplete(
            '../datasets/molecule_datasets_regression/' + args.dataset, dataset=args.dataset,
            transform=MaskAtom(num_atom_type=119, num_edge_type=5,
                               mask_rate=args.mask_rate, mask_edge=args.mask_edge))
    ...
    # set up model
    molecule_model = GNNComplete(args.num_layer, args.emb_dim, JK=args.JK, drop_ratio=args.dropout_ratio, gnn_type=args.gnn_type).to(device)
